...
/*---------------------------------------------------------------------------------------------------------------------------------------------------------*/
comment "Copy selected object and move it on the Y-Axis by its width 30 times";
_obj = get3DENSelected "object" select 0; _pos = getPos _obj; _rot = _obj get3DENAttribute "rotation" select 0; _bbox = boundingBoxReal _obj; _ySize = abs((_bbox select 1 select 1) - (_bbox select 0 select 1)); for "_i" from 1 to 30 do { _newPos = [_pos select 0, (_pos select 1) + (_ySize), _pos select 2]; _newObj = create3DENEntity ["Object", typeOf _obj, _newPos]; _pos = getPos _newObj; };
/*---------------------------------------------------------------------------------------------------------------------------------------------------------*/
